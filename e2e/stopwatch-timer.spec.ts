...
test.describe('Stopwatch Timer - Positive Tests', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/timer/stopwatch');
  });
  ...
  test('DoD: Lap recording functionality', async ({ page }) => {
    ...
    // Click lap button
    const lapButton = page.getByRole('button', { name: /lap/i });
...
test.describe('Stopwatch Timer - Negative Tests', () => {
  ...
  test('Negative: No crash on excessive button clicks', async ({ page }) => {
    ...
    const lapButton = page.getByRole('button', { name: /lap/i });
    for (let i = 0; i < 10; i++) {
      await lapButton.click({ force: true });
    }
  ...
  test('Negative: Lap button should not add laps when not running', async ({ page }) => {
    // Try to click lap before starting
    const lapButton = page.getByRole('button', { name: /lap/i });
    await lapButton.click();
    
    // No laps should be shown (or lap button disabled)
    const lapItems = await page.locator('[class*="lapItem"]').count();
    expect(lapItems).toBe(0);
  });
