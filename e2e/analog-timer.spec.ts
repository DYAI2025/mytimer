...
test.describe('Analog Timer - Positive Tests', () => {
  ...
  test('DoD: Does timer count down?', async ({ page }) => {
    const timeDisplay = page.locator('[class*="time"], [class*="hms"]').first();
  ...
  test('DoD: Does timer stop on pause?', async ({ page }) => {
    ...
    // Should show status as paused
    const status = page.locator('text=/paused/i');
  ...
  test('DoD: Can adjust timer with preset buttons?', async ({ page }) => {
    const presets = ['5m', '10m', '15m', '20m', '25m', '30m'];
...
test.describe('Analog Timer - Negative Tests', () => {
  ...
  test('Negative: Custom input while running should be disabled', async ({ page }) => {
    const startButton = page.getByRole('button', { name: /start/i });
    await startButton.click();
    
    const customInput = page.locator('input[type="number"]').first();
    const isDisabled = await customInput.isDisabled().catch(() => false);
    
    // Input should be disabled while running
    if (!isDisabled) {
      // If not disabled, entering value should not crash
