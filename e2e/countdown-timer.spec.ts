...
test.describe('Countdown Timer - Positive Tests', () => {
  ...
  test('DoD: Does Timer run in normal time speed?', async ({ page }) => {
    ...
    const oneMinPreset = page.getByRole('button', { name: '1 min' });
  ...
  test('DoD: Does Timer count backwards (countdown)?', async ({ page }) => {
    // Set 5 minute timer
    const fiveMinPreset = page.getByRole('button', { name: '5 min' });
  ...
  test('DoD: Is it possible to adjust the timer start value?', async ({ page }) => {
    // Test preset buttons
    const presets = ['1 min', '5 min', '10 min', '15 min', '25 min', '30 min'];
    ...
    for (const preset of presets) {
      ...
      // Verify the preset is selected (has active class or similar)
      const isActive = await presetButton.evaluate(el => 
        el.classList.contains('active') || el.getAttribute('aria-pressed') === 'true'
      );
      // Not all presets may have visual feedback, so this is optional
    }
...
test.describe('Countdown Timer - Negative Tests', () => {
  ...
  test('Negative: UI should not break with unrealistic values', async ({ page }) => {
    ...
    const timerDisplay = page.locator('[class*="time"], [class*="display"]').first();
